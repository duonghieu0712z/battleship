...
cc.Class({
    ...
    onLoad() {
        ...
        // Emitter.instance.registerEvent('', (out, row, column)=>{
        //    out.state= this.map[row][column].state;
        // })
    },
    ...
    addSelectedTile(data) {
        ...
        Emitter.instance.emit('setAvailable', { isAvailable: this.isAvailable, shipId: data.shipId });
    },
    ...
    setShipId(data) {
        ...
        data.positions.forEach(element => {
            this.map[element.y][element.x].getComponent("Tile").shipId = data.shipId;
        })
    },
    checkTile(data){
       if(this.playerId==data.playerId){
        ...
        Emitter.instance.emit('receiveresult',{playerId:this.playerId,worldPosition:targetPosition.mul(-1),shipId:shipId,position:data.position});
        cc.tween(this.node)
        .delay(3)
        .call(()=>{
